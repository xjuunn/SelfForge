...
9. PR 闸门：任何会写入仓库文件、状态或归档的任务必须在独立任务分支中完成，并通过 Pull Request 审查、验证和受保护合并进入 `master`，禁止直接在 `master` 上编辑或本地绕过 PR 合并。
...
14. 更新版本信息。
15. 在任务分支提交代码。
16. 推送任务分支并创建 Pull Request。
17. 等待 PR 审查、自动检查和合并保护全部通过。
18. 通过仓库平台合并 PR 后同步 `master` 并清理任务分支。
19. 启动或准备候选版本。
20. 验证候选版本。
21. 成功后提升版本。
22. 失败后回滚。
...
# 八、分支、Pull Request 与合并规范

任何会修改仓库文件、状态文件、forge 归档或工作区产物的操作，都必须遵守独立任务分支和 Pull Request 流程。只读检查、状态查询、记忆读取和错误查询可以在 `master` 上执行，但首次写入前必须已经离开 `master`。`master` 必须视为受保护分支，只允许通过仓库平台的 PR 合并策略进入，禁止 AI 在线程本地直接合并、强推或绕过审查。
...
9. 创建 PR 前必须重新同步基础分支：执行 `git fetch origin master`，若远端 `master` 有更新，必须先将任务分支变基到最新 `origin/master` 或按仓库策略合并最新 `master`，解决冲突后重新运行验证。
10. 任务分支必须推送到远端：`git push -u origin codex/<短目标>`；推送失败时必须停止并说明原因，禁止改为本地直接合并。
11. 推送成功后必须创建 Pull Request，目标分支为 `master`，标题必须使用 Git 提交规范并包含版本号，正文必须包含任务编号、分支名、目标摘要、主要变更、验证命令和结果、风险评估、回滚方案、归档路径和需要审查的重点。
12. PR 必须保持小而聚焦，一个 PR 只处理一个已领取任务或一个明确目标；发现无关变更必须拆分到新的任务分支和新的 PR。
13. PR 必须等待自动检查通过，并按仓库要求获得至少一名维护者或代码所有者审查；所有审查意见必须处理或明确记录结论，禁止在未解决评论、失败检查或冲突存在时合并。
14. PR 合并必须通过仓库平台执行，合并策略遵守仓库保护规则，可以使用 merge commit、squash 或 rebase，但必须保留版本号、任务编号和可追溯 PR 链接；禁止 AI 在本地执行 `git merge` 后直接推送 `master`。
15. PR 合并后必须在本地切回 `master`，执行 `git pull --ff-only` 同步合并结果，并至少重新执行 `cargo run -- validate`、`cargo run -- preflight` 和 `cargo run -- errors --current --open`；高风险代码变更还必须重新执行 `cargo test`。
16. 只有 PR 已合并且本地 `master` 验证通过后，才允许删除本地任务分支；远端任务分支必须通过平台自动删除或使用受控命令删除。PR 未合并、已关闭或验证失败时，禁止删除任务分支，必须保留现场用于修复、复审或回滚。
17. 若无法创建 PR，例如远端不可用、认证失败或权限不足，必须保留任务分支和提交，输出失败原因和可复现命令，禁止退回到本地直接合并。
18. 禁止在同一个任务分支中处理多个不相关目标；后续目标必须从最新 `master` 重新创建新分支和新 PR。

分支、PR 与协作任务板必须一一对应。一个 AI 线程只能在当前任务分支内处理当前已领取任务的写入范围；需要切换任务时，必须先完成或释放当前任务，再从最新 `master` 创建新分支和新 PR。发现分支名、任务编号、写入范围或当前工作树不一致时，必须停止写入并释放任务，禁止凭聊天记录继续推进。

分支流程检查必须优先使用 `branch-check [--current|--candidate|--version VERSION] [--worker ID] [--task TASK_ID] [--base BRANCH]`。该命令只能读取 Git 状态、当前分支、基础分支、协作任务板和可选任务领取信息，禁止修改文件、切换分支、领取任务、完成任务、删除分支、创建 PR 或执行合并。检查结果必须明确当前分支是否为 `master`、工作区是否存在非任务板未提交改动、基础分支是否可解析、当前分支是否领先或落后基础分支、任务板是否存在、指定 worker 是否持有指定任务，以及是否允许继续写入。协作任务板文件的领取、完成、释放和重开事件属于允许的审计改动，但其他未提交改动必须视为阻断。若检查发现仍在 `master`、工作区存在非任务板未提交改动、指定任务未被当前 worker 领取、任务分支与任务编号不一致或基础分支不可读，AI 必须先修复阻断项，禁止继续写入。

...
6. `agent-work-status` 对待领取和已领取任务显示建议分支名；该提示只读，不能替代实际 `git switch -c`。
...
1. 每次收到新任务，先判断它是否属于当前分支任务组。
2. 仅当目标、写入范围、验收标准和归档主题都明显相关时，才继续使用当前分支。
3. 完全无关任务不得塞进当前分支，也不得混入当前 PR。
4. 若当前分支已有本地成果且任务板可收束，必须先提升 patch、统一 push 一次并创建 PR。
5. 当前 PR 合并并同步 `master` 后，再为无关任务创建新的 `codex/` 分支。
6. 若当前分支没有需要保留的成果，切回最新 `master` 后直接为无关任务创建新分支。
7. 判断为无关任务时，回复必须说明“先收束当前分支”或“已切换新分支”的原因。

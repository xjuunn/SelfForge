...
cargo run -- resolve-error --current --run-id RUN_ID
```
...
通过环境变量提供密钥：`OPENAI_API_KEY`、`DEEPSEEK_API_KEY`、`GEMINI_API_KEY` 或 `GOOGLE_API_KEY`。可用 `SELFFORGE_AI_PROVIDER` 指定 `openai`、`deepseek` 或 `gemini`。

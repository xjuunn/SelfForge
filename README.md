...
cargo test
cargo run -- preflight
cargo run -- branch-check --suggest --worker ai-1 --task coord-002-application
...
cargo run -- agent-work-status # 待领取和已领取任务会显示建议分支名
cargo run -- agent-work-status --active-only # 只显示待领取和已领取任务
...
cargo run -- agent-work-block TASK_ID --reason "阻断原因"

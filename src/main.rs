...
fn main() {
    ...
    let result: Result<String, Box<dyn Error>> = match command.as_str() {
        ...
        "agent-patch-source-cycle-summary" => {
            agent_patch_source_cycle_summary(&app, args.collect())
        }
        "agent-patch-source-cycle-summaries" => {
            agent_patch_source_cycle_summaries(&app, args.collect())
        }
        ...
        "agent-patch-source-task-audit" => agent_patch_source_task_audit(&app, args.collect()),
        "agent-patch-source-task-audits" => agent_patch_source_task_audits(&app, args.collect()),
        "agent-patch-source-task-audit-record" => {
            agent_patch_source_task_audit_record(&app, args.collect())
        }
...
fn agent_work_block(app: &SelfForgeApp, arguments: Vec<String>) -> Result<String, Box<dyn Error>> {
    let command = parse_agent_work_block_args(arguments)?;
    boxed(
        app.block_agent_work(&command.version, &command.task_id, &command.text)
            .map(|report| {
                let mut lines = vec![format!(
                    "SelfForge 协作任务已阻断 版本 {} 任务 {} 文件 {}",
                    report.version,
                    command.task_id,
                    report.queue_path.display()
                )];
...
fn agent_patch_source_cycle_summary(
    app: &SelfForgeApp,
    arguments: Vec<String>,
) -> Result<String, Box<dyn Error>> {
    let command = parse_agent_patch_source_cycle_summary_args(arguments)?;
    boxed(
        app.ai_patch_source_cycle_summary(&command.version, &command.cycle_id)
            .map(|report| {
                format!(
                    "SelfForge AI 补丁源码覆盖 cycle 后续总结完成 版本 {} cycle {} 状态 {} 当前稳定 {} 记忆压缩 {} 下一目标 {} Markdown {} JSON {}",
                    report.record.version,
                    report.record.cycle_id,
                    report.record.status,
                    report.record.stable_version_after,
                    report.record.memory_compaction_recommended,
                    report.record.next_goal,
...
fn agent_patch_source_cycle_summaries(
    app: &SelfForgeApp,
    arguments: Vec<String>,
) -> Result<String, Box<dyn Error>> {
    let command = parse_agent_patch_source_cycle_summaries_args(arguments)?;
    boxed(
        app.ai_patch_source_cycle_summary_records(&command.version, command.limit)
            .map(|records| {
                if records.is_empty() {
                    return format!(
                        "SelfForge AI 补丁源码覆盖 cycle 后续总结 {}: no records",
                        command.version
                    );
                }

                let mut lines = vec![format!(
                    "SelfForge AI 补丁源码覆盖 cycle 后续总结 {}: {} record(s)",
                    command.version,
                    records.len()
                )];
                for record in records {
                    lines.push(format!(
                        "{} 状态 {} cycle {} 当前稳定 {} 下一目标 {} Markdown {} JSON {}",
                        record.id,
                        record.status,
                        record.cycle_id,
                        record.stable_version_after,
...
fn agent_patch_source_cycle_summary_record(
    app: &SelfForgeApp,
    arguments: Vec<String>,
) -> Result<String, Box<dyn Error>> {
    let command = parse_agent_patch_source_cycle_summary_record_args(arguments)?;
    boxed(
        app.ai_patch_source_cycle_summary_record(&command.version, &command.id)
            .map(|record| {
                let mut lines = vec![format!(
                    "SelfForge AI 补丁源码覆盖 cycle 后续总结 {} 版本 {} cycle {} 状态 {} 当前稳定 {} 记忆压缩 {} Markdown {} JSON {}",
                    record.id,
                    record.version,
                    record.cycle_id,
                    record.status,
                    record.stable_version_after,
                    record.memory_compaction_recommended,
                    record.markdown_file.display(),
                    record.file.display()
                )];
                lines.push(format!("下一目标 {}", record.next_goal));
                lines.push(format!("下一任务 {}", record.next_task));
                for command in &record.follow_up_commands {
                    lines.push(format!("后续命令 {command}"));
                }
                lines.join("\n")
...
fn agent_patch_source_task_audit(
    app: &SelfForgeApp,
    arguments: Vec<String>,
) -> Result<String, Box<dyn Error>> {
    let command = parse_agent_patch_source_task_audit_args(arguments)?;
    boxed(
        app.ai_patch_source_task_audit(&command.version, &command.task_draft_id)
            .map(|report| {
                format!(
                    "SelfForge AI 补丁源码覆盖任务草案审计完成 版本 {} 任务草案 {} 状态 {} 发现 {} 建议目标版本 {} 标题 {} Markdown {} JSON {}",
                    report.record.version,
                    report.record.task_draft_id,
                    report.record.status,
                    report.record.findings.len(),
                    report.record.suggested_target_version,
                    report.record.proposed_task_title,
                    report.record.markdown_file.display(),
                    report.record.file.display()
...
fn agent_patch_source_task_audits(
    app: &SelfForgeApp,
    arguments: Vec<String>,
) -> Result<String, Box<dyn Error>> {
    let command = parse_agent_patch_source_task_audits_args(arguments)?;
    boxed(
        app.ai_patch_source_task_audit_records(&command.version, command.limit)
            .map(|records| {
                if records.is_empty() {
                    return format!(
                        "SelfForge AI 补丁源码覆盖任务草案审计 {}: no records",
                        command.version
                    );
                }

                let mut lines = vec![format!(
                    "SelfForge AI 补丁源码覆盖任务草案审计 {}: {} record(s)",
                    command.version,
                    records.len()
                )];
                for record in records {
                    lines.push(format!(
                        "{} 状态 {} 任务草案 {} 建议目标版本 {} 标题 {} Markdown {} JSON {}",
                        record.id,
                        record.status,
...
fn agent_patch_source_task_audit_record(
    app: &SelfForgeApp,
    arguments: Vec<String>,
) -> Result<String, Box<dyn Error>> {
    let command = parse_agent_patch_source_task_audit_record_args(arguments)?;
    boxed(
        app.ai_patch_source_task_audit_record(&command.version, &command.id)
            .map(|record| {
                let mut lines = vec![format!(
                    "SelfForge AI 补丁源码覆盖任务草案审计 {} 版本 {} 任务草案 {} 状态 {} 建议目标版本 {} Markdown {} JSON {}",
                    record.id,
                    record.version,
                    record.task_draft_id,
                    record.status,
                ...
                lines.push(format!("批准目标 {}", record.approved_goal));
                if let Some(reason) = &record.blocked_reason {
                    lines.push(format!("阻断原因 {reason}"));
                }
                for finding in &record.findings {
                    lines.push(format!(
                        "审计发现 {} {} {}",
                        finding.check,
                        if finding.passed { "通过" } else { "未通过" },
                        finding.message
                    ));
                }
                for command in &record.follow_up_commands {
...
struct AgentPatchSourceCycleSummaryArgs {
    version: String,
    cycle_id: String,
}

struct AgentPatchSourceCycleSummariesArgs {
    version: String,
    limit: usize,
}
...
struct AgentPatchSourceTaskAuditArgs {
    version: String,
    task_draft_id: String,
}

struct AgentPatchSourceTaskAuditsArgs {
    version: String,
    limit: usize,
}

struct AgentPatchSourceTaskAuditRecordArgs {
    version: String,
    id: String,
}
...
fn parse_agent_patch_source_cycle_summary_args(
    arguments: Vec<String>,
) -> Result<AgentPatchSourceCycleSummaryArgs, Box<dyn Error>> {
    let state = ForgeState::load(env::current_dir()?)?;
    let mut version = state.current_version.clone();
    ...
    while index < arguments.len() {
        match arguments[index].as_str() {
            ...
            other if other.starts_with("--") => {
                return Err(format!("未知 agent-patch-source-cycle-summary 参数: {other}").into());
            }
            other => {
                if cycle_id.is_some() {
                    return Err("agent-patch-source-cycle-summary 只允许一个 cycle 记录编号".into());
                }
                cycle_id = Some(other.to_string());
                index += 1;
            }
        }
    }

    Ok(AgentPatchSourceCycleSummaryArgs {
        version,
        cycle_id: cycle_id.ok_or("agent-patch-source-cycle-summary 需要 cycle 记录编号")?,
    })
}

fn parse_agent_patch_source_cycle_summaries_args(
    arguments: Vec<String>,
) -> Result<AgentPatchSourceCycleSummariesArgs, Box<dyn Error>> {
    let state = ForgeState::load(env::current_dir()?)?;
    let mut version = state.current_version.clone();
    let mut limit = 10;
    let mut index = 0;
    ...
    while index < arguments.len() {
        match arguments[index].as_str() {
            ...
            other => {
                return Err(
                    format!("未知 agent-patch-source-cycle-summaries 参数: {other}").into(),
                );
            }
        }
    }

    Ok(AgentPatchSourceCycleSummariesArgs { version, limit })
}

fn parse_agent_patch_source_cycle_summary_record_args(
    arguments: Vec<String>,
) -> Result<AgentPatchSourceCycleSummaryRecordArgs, Box<dyn Error>> {
    let state = ForgeState::load(env::current_dir()?)?;
    let mut version = state.current_version.clone();
    let mut id = None;
    let mut index = 0;
    ...
    while index < arguments.len() {
        match arguments[index].as_str() {
            ...
            other if other.starts_with("--") => {
                return Err(
                    format!("未知 agent-patch-source-cycle-summary-record 参数: {other}").into(),
                );
            }
            other => {
                if id.is_some() {
                    return Err("agent-patch-source-cycle-summary-record 只允许一个记录编号".into());
                }
                id = Some(other.to_string());
                index += 1;
            }
...
fn parse_agent_patch_source_task_audit_args(
    arguments: Vec<String>,
) -> Result<AgentPatchSourceTaskAuditArgs, Box<dyn Error>> {
    let state = ForgeState::load(env::current_dir()?)?;
    let mut version = state.current_version.clone();
    let mut task_draft_id = None;
    let mut index = 0;
    ...
    while index < arguments.len() {
        match arguments[index].as_str() {
            ...
            other if other.starts_with("--") => {
                return Err(format!("未知 agent-patch-source-task-audit 参数: {other}").into());
            }
            other => {
                if task_draft_id.is_some() {
                    return Err("agent-patch-source-task-audit 只允许一个任务草案编号".into());
                }
                task_draft_id = Some(other.to_string());
                index += 1;
            }
        }
    }

    Ok(AgentPatchSourceTaskAuditArgs {
        version,
        task_draft_id: task_draft_id.ok_or("agent-patch-source-task-audit 需要任务草案编号")?,
    })
}

fn parse_agent_patch_source_task_audits_args(
    arguments: Vec<String>,
) -> Result<AgentPatchSourceTaskAuditsArgs, Box<dyn Error>> {
    let state = ForgeState::load(env::current_dir()?)?;
    let mut version = state.current_version.clone();
    let mut limit = 10;
    let mut index = 0;
    ...
    while index < arguments.len() {
        match arguments[index].as_str() {
            ...
            other => {
                return Err(format!("未知 agent-patch-source-task-audits 参数: {other}").into());
            }
        }
    }

    Ok(AgentPatchSourceTaskAuditsArgs { version, limit })
}

fn parse_agent_patch_source_task_audit_record_args(
    arguments: Vec<String>,
) -> Result<AgentPatchSourceTaskAuditRecordArgs, Box<dyn Error>> {
    let state = ForgeState::load(env::current_dir()?)?;
    let mut version = state.current_version.clone();
    let mut id = None;
    let mut index = 0;
    ...
    while index < arguments.len() {
        match arguments[index].as_str() {
            ...
            other if other.starts_with("--") => {
                return Err(
                    format!("未知 agent-patch-source-task-audit-record 参数: {other}").into(),
                );
            }
            other => {
                if id.is_some() {
                    return Err("agent-patch-source-task-audit-record 只允许一个记录编号".into());
                }
                id = Some(other.to_string());
                index += 1;
            }
    ...
    Ok(AgentPatchSourceTaskAuditRecordArgs {
        version,
        id: id.ok_or("agent-patch-source-task-audit-record 需要记录编号")?,
    })
}
...
fn parse_agent_work_block_args(
    arguments: Vec<String>,
) -> Result<AgentWorkBlockArgs, Box<dyn Error>> {
    let state = ForgeState::load(env::current_dir()?)?;
    let mut version = state.current_version.clone();
    let mut task_id = None;
    ...
    while index < arguments.len() {
        match arguments[index].as_str() {
            ...
            "--reason" => {
                let Some(value) = arguments.get(index + 1) else {
                    return Err("--reason 需要阻断原因".into());
                };
                text = value.clone();
                index += 2;
            }
            other if other.starts_with("--") => {
                return Err(format!("未知 agent-work-block 参数: {other}").into());
            }
            value => {
                if task_id.is_some() {
                    return Err(format!("未知 agent-work-block 参数: {value}").into());
                }
    ...
    let task_id = task_id.ok_or("agent-work-block 需要任务编号")?;
    Ok(AgentWorkBlockArgs {
        version,
        task_id,
        text,
    })
...
agent-patch-source-task-audit [--current|--candidate|--version VERSION] TASK_DRAFT_ID
agent-patch-source-task-audits [--current|--candidate|--version VERSION] [--limit N]
agent-patch-source-task-audit-record [--current|--candidate|--version VERSION] TASK_AUDIT_ID
